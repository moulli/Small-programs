function h5switch_auditory(Fpath, nfolder, information, overwrite)
...
%     if ~isfield(information, 'increment'); error('Please provide increment distance.'); end
    if ~isfield(information, 'line'); information.line = ''; end
    if ~isfield(information, 'age'); information.age = ''; end
...
%     filetemp = strsplit(ls(Fpath.dir('HDF5')));
...
%         end
%     end
%     h5old_G = [Fpath.dir('HDF5'), '/', filetemp];
    h5old_G = Fpath;
    ...
    % Acquisition:
    h5writeatt(nfolder.filename, '/Metadata', 'Acquisition rate (Hz)', 1 / mean(gradient(h5read(h5old_G, '/Data/Times'))))
    coord = h5read(h5old_G, '/Data/Coordinates');
    h5writeatt(nfolder.filename, '/Metadata', 'Acquisition number of layers', uint8(length(unique(coord(:, 3)))))
    h5writeatt(nfolder.filename, '/Metadata', 'Acquisition increment (µm)', mean(gradient(unique(coord(:, 3)))))
    % Stimulus:
    for i = 1:n1(2)
    ...
    % Coordinates:
    h5fill(h5old_G, '/Data/Coordinates', nfolder.filename, '/Data/Brain/Coordinates', 'single', {'unit', 'mm'; 'space', 'LPS'})
    % Reference coordinates:
    h5fill(h5old_G, '/Data/ZbrainCoordinates', nfolder.filename, '/Data/Brain/ZBrainCoordinates', 'single', {'unit', 'mm'; 'space', 'LPS'; 'reference brain', 'zbrain atlas'})
    % Labels:
    ...
    % Stimulus:
    for i = 1:n1(2)
        h5fill(h5old_G, '/Data/stimulus', nfolder.filename, ['/Data/Stimulus/', information.stimulus.name{i}, '/acousticPulse'], 'single', {'unit', 'dB (?)'})
    end
    
    % BSD:
    try
        h5fill(h5old_G, '/Data/deconv_spikes', nfolder.filename, '/Data/Brain/Analysis/InferredSpikes', 'single')
        h5fill(h5old_G, '/Data/thresh_spikes', nfolder.filename, '/Data/Brain/Analysis/ThresholdedSpikes', 'single')
    catch
